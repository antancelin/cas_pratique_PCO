...
class ImagePredictionApp:
    ...
    def display_user(self, user_prediction):
        ...
        st.write(f"{nom} ({genre})")
        st.write(f"Embauché(e) en {annee_embauche}")
        st.write(f"Au poste de {poste}")
    ...
    def predict_image(self, image):
        ...
        st.write(f"{fiability}% de fiabilité qu'il s'agisse de la bonne personne.")
        if eye_type == 'right':
            st.write(f"Oeil droit utilisé pour la prédiction.")
        elif eye_type == 'left':
            st.write(f"Oeil gauche utilisé pour la prédiction.")
...
if __name__ == "__main__":
    ...
    uploaded_file = st.file_uploader("Sélectionner une image", type=["jpg", "jpeg", "png", "bmp"])
